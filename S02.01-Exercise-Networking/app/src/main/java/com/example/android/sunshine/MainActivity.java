...
//        }
    ...
    public void loadWeatherData() {
        String lugar = SunshinePreferences.getPreferredWeatherLocation(this);
        new WeatherTask().execute(lugar);
    }
    ...
    public class WeatherTask extends AsyncTask<String, Void, String[]> {
        ...
        protected String[] doInBackground(String... direccion) {
            ...
            if (direccion.length != 0) {
                String localizacion = direccion[0];
                try {
                    URL url = NetworkUtils.buildUrl(localizacion);
                    String resultadoJson = NetworkUtils.getResponseFromHttpUrl(url);
                    resultados = OpenWeatherJsonUtils.getSimpleWeatherStringsFromJson(MainActivity.this, resultadoJson);
                } catch (Exception e) {
                    e.printStackTrace();
                }
                    }
        ...
        protected void onPostExecute(String[] resultados) {
            if (resultados != null) {
                for (String resultado : resultados) {
                    mWeatherTextView.append(resultado + "\n");
                }
            }
        }
    }
